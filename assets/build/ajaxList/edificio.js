...
function getSeminarios() {
    ...
    xhttp.open('GET', 'http://localhost/practicaslab_utec/apis/admin/Edificio_api/listEdificios', true);
    xhttp.send();
    xhttp.onreadystatechange=function() {
        ...
        if(this.readyState==4 && this.status ==200){
            ...
            for(let item of datos){
                ...
                <tr>
                <td>${item.edf_codigo}</td>
                <td>${item.edf_nombre}</td>
                <td>${item.edf_acronimo}</td>
                <td>
